          ...
          <div class="card-body">
            ...
            <table class="table table-sm" v-if="arrayData.length">
              ...
              <tbody v-for="(obj, index) in arrayData" :key="index">
                <tr v-if="obj.id">
                  <td>{{ obj.name }}</td>
                  <td>{{ obj.active == 1 ? 'Yes' : 'No' }}</td>
                  <td class="text-right">
                    <router-link
                      :to="{ name: 'Edit Service', params: { id: obj.id } }"
                      class="btn btn-success btn-sm"
                      >Edit</router-link
                    >
...
export default {
  ...
  methods: {
    Exclude: function (driver,index) {
      ...
      this.$http.delete("/api/services/" + driver.id).then(function (e) {
    ...
    getData(search = "") {
      this.$http.get("/api/services?search=" + search).then((e) => {
