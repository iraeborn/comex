...
export default {
  ...
  mounted: function () {
    this.$http.get("/api/signatures/" + this.$route.params.id).then(function (e) {
      this.$data.signature = e.body.signature;
    });
  },
  methods: {
    ...
    Store: function () {
      ...
      this.$data.error = {};
      this.$http
        .put("/api/signatures/"+this.$route.params.id, this.$data.signature)
        .then(function (e) {
          if (!e.body.success) {
