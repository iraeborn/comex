    ...
    <table class="table table-sm table-container" v-else>
        ...
        <tbody v-for="(order, index) in page_orders">
            ...
            <template v-if="$parent.$route.query.status == 'missing_docs'">
                ...
                <td class="text-center">
                    <a v-if="order.drafts && order.drafts.certificate_weight" class="btn btn-success" :href="order.drafts.certificate_weight" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                    <span v-else-if="order.required_docs && order.required_docs.certificate_weight" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    <span v-else class="btn btn-info disabled" title="Document missing"><i class="fas fa-minus"></i></span><br/><br/>

                    <a v-if="order.originals && order.originals.certificate_weight" class="btn btn-success" :href="order.originals.certificate_weight" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                    <span v-else-if="order.required_docs && order.required_docs.certificate_weight" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    <span v-else class="btn btn-info disabled" title="Document missing"><i class="fas fa-minus"></i></span>
                </td>
                <td class="text-center">
                    <a v-if="order.drafts && order.drafts.certificate_quality" class="btn btn-success" :href="order.drafts.certificate_quality" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                    <span v-else-if="order.required_docs && order.required_docs.certificate_quality" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    <span v-else class="btn btn-info disabled" title="Document missing"><i class="fas fa-minus"></i></span><br/><br/>

                    <a v-if="order.originals && order.originals.certificate_quality" class="btn btn-success" :href="order.originals.certificate_quality" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                    <span v-else-if="order.required_docs && order.required_docs.certificate_quality" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    <span v-else class="btn btn-info disabled" title="Document missing"><i class="fas fa-minus"></i></span>
                </td>
                <td class="text-center">
                    <a v-if="order.drafts && order.drafts.report" class="btn btn-success" :href="order.drafts.report" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                    <span v-else-if="order.required_docs && order.required_docs.report" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    <span v-else class="btn btn-info disabled" title="Document missing"><i class="fas fa-minus"></i></span><br/><br/>

                    <a v-if="order.originals && order.originals.report" class="btn btn-success" :href="order.originals.report" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                    <span v-else-if="order.required_docs && order.required_docs.report" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    <span v-else class="btn btn-info disabled" title="Document missing"><i class="fas fa-minus"></i></span>
                </td>
                <td class="text-center">
                    <a v-if="order.drafts && order.drafts.health_certificate" class="btn btn-success" :href="order.drafts.health_certificate" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                    <span v-else-if="order.required_docs && order.required_docs.health_certificate" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    <span v-else class="btn btn-info disabled" title="Document missing"><i class="fas fa-minus"></i></span><br/><br/>

                    <a v-if="order.originals && order.originals.health_certificate" class="btn btn-success" :href="order.originals.health_certificate" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                    <span v-else-if="order.required_docs && order.required_docs.health_certificate" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    <span v-else class="btn btn-info disabled" title="Document missing"><i class="fas fa-minus"></i></span>
                </td>
                <td class="text-center">
                    <a v-if="order.drafts && order.drafts.non_gmo_certificate" class="btn btn-success" :href="order.drafts.non_gmo_certificate" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                    <span v-else-if="order.required_docs && order.required_docs.non_gmo_certificate" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    <span v-else class="btn btn-info disabled" title="Document missing"><i class="fas fa-minus"></i></span><br/><br/>

                    <a v-if="order.originals && order.originals.non_gmo_certificate" class="btn btn-success" :href="order.originals.non_gmo_certificate" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                    <span v-else-if="order.required_docs && order.required_docs.non_gmo_certificate" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    <span v-else class="btn btn-info disabled" title="Document missing"><i class="fas fa-minus"></i></span>
                </td>
                <td class="text-right">
...
<style scoped>
    .table .table-amarelo th,
    .table .table-amarelo td {
...
export default {
    ...
    methods: {
        ...
        Save: function ( order ) {
            ...
            let certificate_weight = order.document[0].certificate_weight;
            let certificate_quality = order.document[0].certificate_quality;
            let report = order.document[0].report;

            let inspection_start_datetime = order.inspection_agency.inspection_start_date + " " + order.inspection_agency.inspection_start_time
            let inspection_end_datetime = order.inspection_agency.inspection_end_date + " " + order.inspection_agency.inspection_end_time

            this.$http.post('/api/orders-inspection', {
                order_id,
                certificate_weight,
                certificate_quality,
                report,
                inspection_start_datetime,
                inspection_end_datetime,
            }).then( e => {
        ...
        _filterOrders: async function() {
            ...
            let response = await this.$http.get('/api/orders-inspection?' + this.getQuery());
            ...
            if (response.body && response.body.orders) {
                ...
                orders = orders.map( (order, key ) => {
                    if (!order.inspection_agency) return order;
                    if(order.inspection_agency.inspection_start_datetime) {
                        order.inspection_agency.inspection_start_date = order.inspection_agency.inspection_start_datetime.match(/^[^ ]+/);
                    }

                    if(order.inspection_agency.inspection_start_datetime) {
                        order.inspection_agency.inspection_start_time = order.inspection_agency.inspection_start_datetime.match(/[^ ]+$/);
                    }

                    if(order.inspection_agency.inspection_end_datetime) {
                        order.inspection_agency.inspection_end_date   = order.inspection_agency.inspection_end_datetime.match(/^[^ ]+/);
                    }

                    if(order.inspection_agency.inspection_end_datetime) {
                        order.inspection_agency.inspection_end_time   = order.inspection_agency.inspection_end_datetime.match(/[^ ]+$/);
                    }
