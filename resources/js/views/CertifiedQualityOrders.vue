  ...
  <div class="card-body">
    ...
    <table class="table table-sm table-container" v-else>
        ...
        <tr v-for="(order, index) in page_orders" v-if="order.document.length > 0">
            ...
            <td>{{ GetStatus(order.document[0].certificate_quality_status) }}</td>
            <td class="text-right">
                    ...
                    <div class="modal-dialog modal-success">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <popcorn-upload-new :allowed_pattern="/^application\/pdf$/" v-model="order.document[0].certificate_quality"></popcorn-upload-new>
                            </div>
...
export default {
    ...
    methods: {
        ...
        Save: function ( order ) {
            let post_data = {
                order_id: order.id,
                document_url: order.document[0].certificate_quality,
            }

            this.$http.post('/api/orders-quality', post_data).then( e => {
        ...
        _filterOrders: async function() {
            ...
            let response = await this.$http.get('/api/orders-quality?' + this.getQuery());
