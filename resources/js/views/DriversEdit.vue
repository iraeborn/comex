...
<script>
  export default {
    ...
    Store: function () {
            this.$data.error = {}
            this.$http.put('/api/drivers/' + this.$route.params.id, this.$data.driver).then(function (e) {
            if (!e.body.success) {
    ...
    mounted: function () {
      this.$http.get('/api/drivers/' + this.$route.params.id).then(e => {
        this.$data.driver = e.body
      })
    }
  }
</script>
