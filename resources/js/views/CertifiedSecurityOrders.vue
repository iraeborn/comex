        ...
        <tbody v-for="(order, index) in page_orders">
            ...
            <template v-else>
                <tr v-if="order.owner">
                    ...
                    <td class="text-center">
                        <a v-if="order.drafts && order.drafts.certificate_seguro" class="btn btn-success" :href="order.drafts.certificate_seguro" target="_blank" title="View document"><i class="fas fa-file"></i></a>
                        <span v-else-if="order.required_docs && order.required_docs.certificate_seguro" class="btn btn-danger disabled" title="Document required"><i class="fas fa-times"></i></span>
                    ...
                    <div class="modal-dialog modal-success">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <popcorn-upload-new :allowed_pattern="/^application\/pdf$/" v-model="order.document[0].certificate_seguro || null"></popcorn-upload-new>
                            </div>
...
export default {
    ...
    methods: {
        ...
        Save: function ( order ) {
            let post_data = {
                order_id: order.id,
                document_url: order.document[0].certificate_seguro || null,
            }

            this.$http.post('/api/orders-seguro', post_data).then( e => {
        ...
        _filterOrders: async function() {
            ...
            let response = await this.$http.get('/api/orders-seguro?' + this.getQuery());
