...
export default {
    name: "assign-contract-order-modal",
    ...
    methods: {
        ...
        updateData(orders) {
            ...
            this.$emit('update-data', { ordersIds });
        },
        ...
        async getOrdersList() {
            try {
                const response = await this.$http.get('/api/list-orders');
                const orders = response.data;

                const ordersData = Array.isArray(orders) ? orders.map((order) => ({
                    id: order.id,
                    number: order.number
                })) : [];
                ...
                console.error('Failed to fetch list orders:', error);
                this.logs = `Failed to fetch list orders: ${error.message}`;
            }
        },
