<template>
  <div class="V2">
    <table class="table table-sm table-amarelo">
      ...
      <tr>
        <td colspan="6">{{ container_stuffing.terminal_observations }}</td>
        <td colspan="6">{{ container_stuffing.terminal_user }}</td>
      </tr>
    ...
    <div class="row">
      ...
      <div class="col text-right">
        <button @click="OpenAddContainerModal" class="btn btn-success btn-sm">Add new container</button>
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <div class="form-group">
                  ...
                  <input-icon type="number" icon="fas fa-box" v-model="current_container.seal" name="seal" :class="error.seal != '' && error.seal ? 'is-invalid' : ''" id="seal" />
          ...
          <tbody v-for="(container, index) in container_stuffing.containers">
            <tr>
              ...
              <td style="width: 150px;">
                <!-- <button class="btn btn-danger btn-sm float-right" style="margin-left: 5px;" @click="DeleteContainer(index)">Remove</button>
                <button class="btn btn-success btn-sm float-right" @click="EditContainer(container, index)">Edit</button> -->
              </td>
            </tr>
            <tr>
              <td colspan="4">
                ...
                <table class="table table-sm" v-if="InContainer(bill_list, container.id).length > 0">
                  ...
                  <tr v-for="(bill, index) in InContainer(bill_list, container.id)">
                    ...
                    <td class="text-right">
                      &nbsp;
                      <!-- <button class="btn btn-danger btn-sm" @click="OpenBillModal(bill, index)">Edit</button> -->
                    </td>
                  </tr>
                </table>
              </td>
    ...
    methods: {
        ...
        Save: function () {
          this.$http.put('/api/containers/' + this.$route.params.id, this.$data.container_stuffing.containers).then(e => {
            ...
            this.$toaster.success('Containers saveds!')
          })
        },
    }
}
