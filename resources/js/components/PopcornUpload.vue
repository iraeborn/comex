<template>
    <div>
        ...
        <input type="file" :name="name + '_field' + data_sufix" :id="name + '_field' + data_sufix" class="popcorn-upload-field" @change="FileSelected">
                    ...
                    <div class="col-sm-12">
                        <div v-if="!IsImage(data_file)">
                            <span class="row">
                                ...
                                <span class="col">
                                    <a :href="data_file.url" target="_blank" v-if="!IsImage(data_file)">
                                        Link
                                    </a>
                                </span>
                            </span>
...
<script>
export default {
  props: ['name', 'allowed_pattern', 'file', 'sufix'],
  name: 'popcorn-upload',
  data () {
    return {
        file_name: null,
  ...
  mounted: function () {
    ...
    this.$data.data_file = {}
    this.$data.data_file.url = this.$props.file
    this.$data.data_sufix = this.$props.sufix
  },
  watch: {
  ...
  methods: {
    ...
    FileUpload: function () {
        ...
        this.$http.post('/api/upload', this.$data.formData, {
            ...
            this.$emit('upload', this.$data.data_file)
        }, (response) => {
            this.$data.ready = false
            this.uploading = false
            this.$data.progress = 0
    ...
    ChoseFile: function (name) {
        document.getElementById(name + '_field' + this.$data.data_sufix).click()
    },
    ...
    RemoveFile: function () {
        ...
        this.$emit('upload', this.$props.file)
    }
  }
}
</script>
